// Calculate days until due date from a date string (YYYY-MM-DD format)
export function calculateDaysUntilDue(dateString: string | null | undefined): number | undefined {
  ...
  try {
    ...
    return Math.max(0, diffDays) // Return 0 if overdue
  } catch {
    return undefined
  }
}
...
// Map priority label (API response) to numeric value (0, 1, 2)
...
// Map numeric priority value to label string
...
// For use with string state (Select components)
